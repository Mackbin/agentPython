...
> 本文是行动计划，不承担项目架构说明。开始前先看 [Autobots 项目资料](../docs/autobots/README.md) 和 [Agent 基础](README.md)，否则会把练习清单当成系统设计文档。

## 使用方式

每周只完成一个闭环：**阅读指定入口 -> 做练习 -> 按验证标准自测 -> 记录不会的内容**。不要把 12 周任务当成必须一次性读完的目录。

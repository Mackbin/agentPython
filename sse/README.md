...
1. [文件定位与作用](#1-文件定位与作用)
2. [模块导入与全局对象](#2-模块导入与全局对象)
3. [SSEEventType 事件类型枚举](#3-sseeventtype-事件类型枚举)
4. [SSEEvent 事件对象](#4-sseevent-事件对象)
5. [SSEStream 流管理器总览](#5-ssestream-流管理器总览)
6. [事件优先级设计](#6-事件优先级设计)
7. [背压丢弃策略(精妙)](#7-背压丢弃策略精妙)
8. [入队逻辑](#8-入队逻辑)
9. [语义化发送方法族](#9-语义化发送方法族)
10. [OUTPUT_DISCARD 撤回机制](#10-outputdiscard-撤回机制)
11. [终结事件与关闭](#11-终结事件与关闭)
12. [通用事件分发](#12-通用事件分发)
13. [异步迭代器(最难)](#13-异步迭代器最难)
14. [响应生成器](#14-响应生成器)
15. [与 FastAPI 的集成](#15-与-fastapi-的集成)
16. [与前端的对应关系](#16-与前端的对应关系)
17. [整体架构图](#17-整体架构图)
18. [学习要点总结](#18-学习要点总结)
19. [前端代码对照](#19-前端代码对照)
20. [自己实现最简版本](#20-自己实现最简版本)
